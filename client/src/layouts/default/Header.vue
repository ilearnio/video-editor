...
import { pocketbaseLogoutUser } from '@/services/pocketbaseAuth'
...
const isDark = useDark({
  onChanged(dark: boolean) {
    userStore.actions.setIsDark(dark)
  },
})
...
function renderCustomHeader() {
  return h(
    ...
    [
      ...
      h('div', null, [
        h('div', null, [h(NText, { depth: 2 }, { default: () => userStore.state.user?.username })]),
        h('div', { style: 'font-size: 12px;' }, [
          h(NText, { depth: 3 }, { default: () => `${userStore.state.user?.email}` }),
        ]),
      ]),
    ],
  )
